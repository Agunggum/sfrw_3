...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load Kamus saat halaman dibuka
    ...
    window.changeLanguage = function(lang) {
        ...
        document.cookie = "user_language=" + lang + ";path=/;max-age=" + (365 * 24 * 60 * 60) + ";SameSite=Lax";

        // 2. Ubah ikon bendera utama di tombol toggle
        const flagMapping = { 'en': 'fi-us', 'id': 'fi-id' };
        const currentFlagEl = document.getElementById('current-flag-icon');
        if (currentFlagEl && flagMapping[lang]) {
            currentFlagEl.classList.remove('fi-id', 'fi-us');
        ...
        // 3. KUNCI PERBAIKAN: Update status Active dan Checklist di Dropdown
        document.querySelectorAll('.lang-item').forEach(item => {
            const itemLang = item.getAttribute('data-lang');
            const checkIcon = item.querySelector('.check-icon');

            if (itemLang === lang) {
                // Beri class active pada list terpilih
                //item.classList.add('active');
                // Munculkan tanda centang dengan menghapus class d-none
                if (checkIcon) checkIcon.classList.remove('d-none');
            } else {
                // Hapus class active dari list yang tidak terpilih
                //item.classList.remove('active');
                // Sembunyikan tanda centang dengan menambah class d-none
    ...
    // Observer untuk mendeteksi perubahan Class atau ID
